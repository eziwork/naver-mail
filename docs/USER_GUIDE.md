...
설치를 맡은 AI는 [설치 도우미 지침](INSTALL_FROM_GITHUB.md)을 읽고 OS별 일회성 도우미를 실행합니다. 도우미는 정식판과 사전 배포판 목록을 확인해 현재 운영체제·CPU에 맞는 운영 패키지를 고릅니다. 이어서 파일별 `.sha256` 또는 `SHA256SUMS.txt`의 SHA-256 값이 일치하는지 확인하고, 압축을 풀어 완성된 폴더를 로컬 플러그인 목록에 등록합니다. 기존 플러그인 설정은 유지하며, 설치 과정에서 네이버 비밀번호를 묻지 않습니다. 설치가 끝나면 앱을 다시 열거나 새 작업을 시작하라는 안내가 표시됩니다.
...
Codex가 등록을 마치면 새 작업을 시작하세요. 새 작업에서도 도구가 나타나지 않을 때만 앱을 다시 시작합니다. **플러그인 → 개인/로컬 목록 → 네이버 메일**에서 설치하고 새 작업을 시작하세요. 메뉴 명칭은 앱 버전에 따라 다를 수 있습니다. 조직 정책으로 로컬 플러그인이 제한된 경우 조직 관리자에게 설치를 요청합니다.
...
입력 화면의 **네이버 보안설정 열기 — 앱 비밀번호 만들기** 버튼으로 새 탭을 엽니다. 네이버 2단계 인증 관리에서 이 연결에 사용할 **애플리케이션 비밀번호**를 만듭니다. 앱 비밀번호는 일반 네이버 로그인 비밀번호와 별개입니다. 발급 과정은 [네이버 공식 앱 비밀번호 안내](https://help.naver.com/service/5640/contents/8584?lang=ko)를 참고하세요.
...
업데이트 전에 기존 압축 패키지를 보관합니다. 설치 도우미로 기존 등록을 갱신한 뒤 새 작업을 시작합니다. 새 작업에서도 도구가 나타나지 않을 때만 앱을 완전히 종료했다 새로 시작합니다. 계정 정보는 OS 보안 저장소에 있어 기존 저장 형식을 유지하는 업데이트에서는 그대로 사용합니다. 재시작하면 발송 준비 내용은 사라집니다.
...
**스마트폰이나 웹 Work에서도 쓸 수 있나요?** 0.2.1은 로컬 데스크톱 환경용입니다. 웹·클라우드 실행 환경과 모바일은 이번 지원 범위에 포함하지 않습니다.
...
설정은 처음 연 시각부터 30분 동안 유지됩니다. 유효한 화면을 다시 요청해도 만료 시간이 늘어나지 않습니다. 앱을 닫은 뒤에도 설정을 계속할 수 있도록 작업 프로세스를 분리했습니다. 실제 앱별 검증 상태는 [배포 기록](../RELEASE.md)을 확인하세요.

새로고침하면 같은 유효 세션의 이메일과 단계를 복원합니다. 비밀번호는 복원하지 않습니다. 입력 중 서버가 끊기면 상태와 복구 안내가 나타납니다. 제출 결과가 불명확하면 자동 재제출하지 않고 상태만 확인합니다. **연결 설정 취소** 버튼으로 취소할 수 있고, 완료·취소·만료 시 임시 진행 정보를 지웁니다. 계정 연결 완료 자체에는 앱 재시작이 필요하지 않습니다.
