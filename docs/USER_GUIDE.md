...
GitHub 저장소만 방문했다고 앱에 플러그인이 자동 설치되지는 않습니다. 먼저 압축을 푼 폴더를 데스크톱 앱의 로컬 플러그인 목록에 등록합니다. 처음 사용하는 분은 Codex의 로컬 작업에서 다음 문장을 복사해 요청하세요. 꺾쇠 부분만 실제 폴더 위치로 바꿉니다.
...
이미 관리자가 배포한 목록에 네이버 메일이 보이면 목록에서 바로 설치하면 됩니다. 이 저장소의 개발 소스를 GitHub 마켓플레이스로 바로 추가하는 방식은 준비된 설치 경로가 아닙니다. 실행에 필요한 파일이 포함된 패키지를 사용해야 합니다. 관리자용 등록 구조는 [개발 문서](DEVELOPMENT.md#로컬-앱에서-설치-검증하기)를 참고하세요.
