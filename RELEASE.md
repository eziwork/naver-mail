...
사용자는 저장소 주소와 아래 한 문장만 Codex 또는 로컬 MCP를 지원하는 데스크톱 Work에 전달하면 됩니다.
...
호스트는 현재 OS·CPU에 맞는 Release 운영 패키지를 선택하고 체크섬을 확인한 뒤 완성된 플러그인 폴더를 등록합니다. 소스 ZIP이나 Release의 **Source code**는 실행 파일과 런타임이 없어 설치에 사용하지 않습니다. 호스트가 주소 설치를 지원하지 않을 때의 수동 절차와 검사 순서는 [사용자 안내](docs/USER_GUIDE.md)와 [GitHub 주소 설치 지침](docs/INSTALL_FROM_GITHUB.md)을 참고하세요.
